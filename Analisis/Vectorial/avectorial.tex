...
\[\int_\Omega\operatorname{div} F_j(x)dx=\int_{U_j}\operatorname{div} F_j(x)dx=\int_{\partial U_j}\langle F_j(x),n(x) \rangle dx=\int_{\partial\Omega}\langle F_j(x),n(x) \rangle dx.\] % Creo que la última igualdad no es cierta en general. Sería cierta al sumar si tomásemos una subdivisión de $\Omega$ en hipercubos disjuntos, algunos de los cuales tendrían una de sus caras como gráfica de una función. En este caso se cancelarían al sumar las integrales en las fronteras interiores (hipersuperficies con distinta ``orientación'') y quedarían las exteriores, la de $\Omega$.
