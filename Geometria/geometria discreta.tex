...
\begin{document}
	...
	\textbf{Demostración:} Lo interesante es la suficiencia, pues ese número de V-guardias es necesario para vigilar el exterior de cualquier polígono convexo.
	
	 Sea $P$ un polígono cualquiera con $n$ lados. Comenzamos haciendo su envolvente convexa y triangulando la parte de su envolvente convexa exterior a $P$, esto nos genera un grafo plano $G''$.
	...
	Ahora abrimos el grafo duplicando el vértice $x$ en $x'$ y $x''$, ambos quedarán conectados con todos los vértices que lo estaban con $x$, conectaremos $V_\infty$ con $x'$ y $x''$ rodeando el grafo por dos lados  diferentes (arriba y abajo o izquierda y derecha). Obtenemos así el grafo $G$, que tiene $n+2$ nodos (hemos añadido $V_\infty$ y duplicado $x$).
	
	\begin{figure}[H]
		\includegraphics[width=80mm]{imgs/museo-g}
	\end{figure}
	
	$G$ que es la triangulación de un polígono (al menos topológicamente hablando) y por tanto 3-coloreable. Coloreamos $G$ con los colores rojo, verde y azul. Habrá un color que esté presente en a lo sumo $E\left[\frac{n+2}{3}\right]$ vértices, pongamos que es el rojo. Si $V_\infty$ no es rojo, entonces colocando guardias en los vértices rojos tendremos como mucho $E\left[\dfrac{n+2}{3}\right]\leq E^+\left[\dfrac{n}{2}\right]$ guardias, y es claro que vigilan todo el exterior del polígono.
	
	Si $V_\infty$ es rojo, nos olvidamos de él y nos quedamos con $n+1$ vértices que serán rojos, verdes o azules. Entre el azul y el verde debe haber un color que aparezca como mucho en $E\left[\dfrac{n+1}{2}\right]=E^+\left[\dfrac{n}{2}\right]$ vértices (este sería el peor de los casos donde el único rojo es $V_\infty$). Colocando guardias en los vértices de ese color vigilamos todo el exterior del polígono y nos aseguramos de que el número de guardias no excede el límite impuesto.
	...
	\textbf{Solución:} Claramente una triangulación es una descomposición por triángulos, luego existen descomposiciones con un número de triángulos menor o igual que el de cualquier triangulación. Se nos pregunta por la desigualdad contraria.
	
	La respuesta a esta pregunta es negativa. Consideremos el siguiente polígono de 8 vértices (los azules).
	
	\begin{figure}[H]
		\includegraphics[width=70mm]{imgs/triangulacion-descomposicion}
	\end{figure}

	Como el polígono tiene 8 vértices, sabemos que toda triangulación suya tiene $8-2=6$ triángulos. Sin embargo, en el polígono aparece marcada una descomposición en 5 triángulos.
	...
	lo cual es absurdo, puesto que $b_{2q}\neq 0$. Concluimos que $\dfrac{\arccos{(1/3)}}{\pi}$ es irracional y que $\{\pi,\arccos{(1/3)}\}$ es una base de $V$.
